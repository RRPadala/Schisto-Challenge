...
#Humt <- mapIds(hgu95av2.db, keys=Humt, column=c("SYMBOL"), keytype="ENSEMBL")
...
#rbgene1 = grep(pattern = "-RP[S|L]", rownames(rna1), value = TRUE)
#rbgene2 = grep(pattern = "-RP[S|L]", rownames(rna2), value = TRUE)
...
#DefaultAssay(seurathuman) <- "RNA"
#seurathuman <- FindNeighbors(seurathuman, reduction = "harmony", verbose = FALSE)
#seurathuman <- RunUMAP(seurathuman, dims = 1:10, verbose = FALSE)
#seurathuman <- FindClusters(seurathuman, graph.name = "RNA_snn", resolution = 1, verbose = FALSE)

#Idents(seurathuman) <- "RNA_snn_res.1"
#p <- DimPlot(seurathuman, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
#ggsave(filename="UMAP_RNA_human.png",plot=p,device="png",dpi=600,path=workdirrna,height=7,width=7)
...
#DefaultAssay(seuratrhesus) <- "RNA"
#seuratrhesus <- FindNeighbors(seuratrhesus, reduction = "harmony", verbose = FALSE)
#seuratrhesus <- RunUMAP(seuratrhesus, dims = 1:10, verbose = FALSE)
#seuratrhesus <- FindClusters(seuratrhesus, graph.name = "RNA_snn", resolution = 1, verbose = FALSE)

#Idents(seuratrhesus) <- "RNA_snn_res.1"
#p <- DimPlot(seuratrhesus, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
#ggsave(filename="UMAP_RNA_rhesus.png",plot=p,device="png",dpi=600,path=workdirrna,height=7,width=7)
